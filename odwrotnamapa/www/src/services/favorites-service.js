(function () {
  ...
  // Zagniezdzanie folderow bez zmiany formatu przechowywania - nazwa
  // folderu moze zawierac "/" jako separator sciezki (np.
  // "Wakacje/Gdansk" to podfolder "Gdansk" wewnatrz "Wakacje").
  // W pelni wstecznie kompatybilne - istniejace foldery bez "/" dzialaja
  // dokladnie jak wczesniej, jako foldery najwyzszego poziomu.
  ...
  function getDirectChildFolders(parentPath) {
    return ctx.state.favoriteFolders.filter(
      f => getFolderParentPath(f) === parentPath
    );
  }

  function createFavoriteFolder() {
    const rawName = (ctx.el.favoritesNewFolderInput?.value || "").trim();
    if (!rawName) return;
    window.OMAP_ACCOUNT?.markLocalSyncChange();
    // Tworzenie folderu, gdy jestesmy juz wewnatrz innego folderu,
    // automatycznie zagniezdza go jako podfolder biezacego.
    const activeFolder = ctx.state.activeFavoriteFolder || "";
    const isRealParent = activeFolder && activeFolder !== ctx.UNFILED_FOLDER;
    const name = isRealParent ? `${activeFolder}/${rawName}` : rawName;
  ...
  function deleteFavoriteFolder(folder) {
    window.OMAP_ACCOUNT?.markLocalSyncChange();
    // Usuwanie kaskaduje na WSZYSTKIE podfoldery (nazwa zaczynajaca
    // sie od "folder/") - inaczej usuniecie folderu z podfolderami
    // zostawialoby "osierocone" podfoldery, wciaz widoczne, ale bez
    // rodzica do ktorego naleza.
    const toDelete = new Set([
      folder,
      ...ctx.state.favoriteFolders.filter(f => f.startsWith(`${folder}/`))
    ]);
    ctx.state.favoriteFolders = ctx.state.favoriteFolders.filter(f => !toDelete.has(f));
    saveFavoriteFolders();

    let changed = false;
    ctx.state.favorites.forEach(favorite => {
      if (toDelete.has(favorite.folder)) {
        favorite.folder = "";
        changed = true;
      }
    });
    ...
    if (toDelete.has(ctx.state.activeFavoriteFolder)) ctx.state.activeFavoriteFolder = "";
    if (toDelete.has(ctx.state.activeRouteFolder)) ctx.state.activeRouteFolder = "";
    renderFolderChips();
    renderFavoritesList();
  }
  ...
  function renameFavoriteFolder(oldName, newName) {
    ...
    // Zmiana nazwy kaskaduje na WSZYSTKIE podfoldery - "Wakacje" ->
    // "Urlop" powinno tez zmienic "Wakacje/Gdansk" na "Urlop/Gdansk",
    // inaczej podfoldery zostalyby "osierocone" pod stara sciezka.
    const oldPrefix = `${oldName}/`;
    const renameMap = new Map();
    renameMap.set(oldName, trimmed);
    ctx.state.favoriteFolders.forEach(f => {
      if (f.startsWith(oldPrefix)) {
        renameMap.set(f, trimmed + f.slice(oldName.length));
      }
    });

    ctx.state.favoriteFolders = ctx.state.favoriteFolders.map(
      f => renameMap.get(f) || f
    );
    saveFavoriteFolders();
    ...
    let changed = false;
    ctx.state.favorites.forEach(favorite => {
      if (renameMap.has(favorite.folder)) {
        favorite.folder = renameMap.get(favorite.folder);
        changed = true;
      }
    });
    if (changed) saveFavorites();
    ...
    let routesChanged = false;
    ctx.state.routeFavorites.forEach(route => {
      if (renameMap.has(route.folder)) {
        route.folder = renameMap.get(route.folder);
        routesChanged = true;
      }
    });
    if (routesChanged) ctx.saveRouteFavorites();

    if (renameMap.has(ctx.state.activeFavoriteFolder)) {
      ctx.state.activeFavoriteFolder = renameMap.get(ctx.state.activeFavoriteFolder);
    }
    if (renameMap.has(ctx.state.activeRouteFolder)) {
      ctx.state.activeRouteFolder = renameMap.get(ctx.state.activeRouteFolder);
    }
    renderFolderChips();
    renderFavoritesList();
  }
  ...
  function renderFavoritesList() {
    ...
    //
    // Foldery moga byc teraz zagniezdzone (nazwa ze "/") - pokazujemy
    // TYLKO bezposrednie dzieci biezacego poziomu (root, jesli
    // activeFolder jest puste, albo podfoldery activeFolder), nie
    // wszystkie foldery naraz plasko.
    const isRealFolder = activeFolder && activeFolder !== ctx.UNFILED_FOLDER;
    const showFolderRows = !query;
    const childFolders = showFolderRows
      ? getDirectChildFolders(isRealFolder ? activeFolder : "")
      : [];
    const hasAny = ctx.state.favorites.length > 0 || ctx.state.routeFavorites.length > 0 ||
      (showFolderRows && childFolders.length > 0);
    const hasMatches = favorites.length > 0 || routes.length > 0 || (showFolderRows && childFolders.length > 0) || (showFolderRows && isRealFolder);
    ...
    // Przycisk "wstecz" pokazuje sie zawsze, gdy jestesmy wewnatrz
    // prawdziwego folderu - NIEZALEZNIE od tego, czy ma on podfoldery
    // do pokazania. Idzie do RODZICA (nie zawsze do "Wszystkie") -
    // przy zagniezdzonych folderach to kolejny krok w gore, nie
    // skok od razu na sam szczyt.
    if (isRealFolder) {
      const parentPath = getFolderParentPath(activeFolder);
      const backRow = document.createElement("button");
      backRow.type = "button";
      backRow.className = "favorite-folder-back-row";
      backRow.textContent = `← ${parentPath ? getFolderDisplayName(parentPath) : t.favoriteFolderAll}`;
      backRow.addEventListener("click", () => {
        ctx.state.activeFavoriteFolder = parentPath;
        renderFolderChips();
        renderFavoritesList();
      });
      attachFolderDropTarget(backRow, parentPath);
    ...
    if (showFolderRows) {
      childFolders.forEach(folder => {
        ...
        renameInput.value = getFolderDisplayName(folder);
        ...
        renameSave.addEventListener("click", () => {
          const parentPath = getFolderParentPath(folder);
          const newFullPath = parentPath
            ? `${parentPath}/${renameInput.value.trim()}`
            : renameInput.value.trim();
          renameFavoriteFolder(folder, newFullPath);
        });
    ...
    const visibleFavorites = (showFolderRows && !isRealFolder)
    ...
    const visibleRoutes = (showFolderRows && !isRealFolder)

(function () {
  ...
  async function publishProfileMetadata(nostrPrivKeyBytes, profile) {
    ...
    // (data:image/...;base64,...), nie URL - moglby byc bardzo dlugi
    // (dziesiatki tysiecy znakow) i obciecie do 2000 znakow by go
    // uszkodzilo, dajac zepsuty obrazek zamiast go nie pokazywac
    // wcale. Standardowe pole "picture" w metadanych Nostr (kind 0)
    // ma byc URL-em, nie danymi inline - publikujemy je wiec TYLKO
    // gdy to faktyczny http(s) adres.
    ...
    const content = JSON.stringify({
      ...
      picture: isRealUrl ? avatarValue.slice(0, 2000) : ""
    });
      ...
      for (const pubkey of uncached) {
        ...
        if (event) {
          try {
            ...
            avatar = String(parsed.picture || "").trim().slice(0, 2000);
          } catch (_) {
            name = "";
            avatar = "";
          }

(function () {
  ...
  async function loadWikipediaSummaryForPlace(place, ui, headingElement) {
    ...
    //
    // WAZNE: uzywamy ctx.getPlaceNameKey (ta sama, biezaca funkcja co
    // reszta appki) - ten kod mial WLASNA, osobna, nigdy nie
    // aktualizowana kopie tej logiki (surowe wspolrzedne, 5 miejsc po
    // przecinku), przez co NIGDY nie rozpoznawala wlasnych nazw
    // zapisanych pod nowym systemem kluczy (poi-name:...) - kazde
    // otwarcie miejsca z wlasna nazwa bylo wiec asynchronicznie
    // nadpisywane tytulem z Wikipedii, kilkaset milisekund po
    // poprawnym poczatkowym wyswietleniu.
    if (data.title && headingElement) {
      const placeNameKey = ctx.getPlaceNameKey ? ctx.getPlaceNameKey(place) : null;

(() => {
  ...
  // Zabezpieczenie samonaprawiajace: jesli CONFIG (z config.js) nie
  // zaladowal sie poprawnie - najbardziej prawdopodobna przyczyna to
  // niespojny/uszkodzony cache Service Workera (config.js i app.js
  // "rozjechane" miedzy sobą w cache'u przegladarki) - reszta appki
  // by i tak natychmiast sie wywalila. Zamiast tego: wyrejestruj
  // Service Workera, wyczysc wszystkie cache, i wymus twarde
  // przeladowanie - bez zadnej akcji ze strony uzytkownika. Flaga w
  // sessionStorage chroni przed nieskonczona petla przeladowan, gdyby
  // prawdziwa przyczyna byla inna (np. config.js faktycznie brakuje
  // na serwerze) - wtedy po JEDNEJ probie appka juz nie probuje
  // ponownie, tylko pokazuje czytelny komunikat.
  if (!CONFIG) {
    const alreadyTriedSelfHeal = window.sessionStorage?.getItem("omap_config_selfheal_attempted");
    if (!alreadyTriedSelfHeal) {
      window.sessionStorage?.setItem("omap_config_selfheal_attempted", "1");
      Promise.all([
      ...
      document.body.innerHTML =
        '<div style="padding:24px;font-family:sans-serif;max-width:480px;margin:60px auto;text-align:center;">' +
        "<h2>Błąd wczytywania</h2>" +
        "<p>Nie udało się poprawnie wczytać konfiguracji strony. Odśwież ręcznie (Ctrl+Shift+R) lub spróbuj ponownie za chwilę.</p>" +
        "</div>";
    }
    return;
  }
  ...
  const text = {
    pl: {
      ...
      ratingRetryHint: "Kliknij, aby spróbować ponownie",
      ratingSaving: "Zapisywanie oceny…",
      ratingDelete: "Usuń ocenę",
      commentsHeading: "Komentarze",
      commentsLoading: "Wczytywanie komentarzy…",
      commentsNone: "Brak komentarzy - bądź pierwszy/a",
      commentsError: "Nie udało się wczytać komentarzy.",
      commentsRetryHint: "Kliknij, aby spróbować ponownie",
      commentsPlaceholder: "Napisz komentarz…",
      commentsSubmit: "Dodaj",
      commentDelete: "Usuń komentarz",
      commentEdit: "Edytuj komentarz",
      commentReply: "Odpowiedz",
      commentAuthorRating: "Ocena autora: {rating}/5",
...
map.on('rotate', updateLogoRotation);
  ...
  window.OMAP_COMMENTS?.configure({
    state,
    text,
    getStoredSeedWords: window.OMAP_SEED_WORDS?.getStoredSeedWords,
    openAccountFromMenu: window.OMAP_ACCOUNT?.openAccountFromMenu,
    setPendingPlaceReturn
  });
  ...
  // Uwaga: OMAP_WEATHER.configure() już wywołane wcześniej (patrz
  // wyżej) i OMAP_WEATHER.init() już wywoływane w map.on("load", ...)
  // - druga kopia configure()+init() tutaj była duplikatem (dodawała
  // drugi listener "moveend" i odpalała podwójne zapytanie do
...
function applyLanguage(language) {
  ...
  // Google mogl juz zaindeksowac linki z bazy sitemap.xml sprzed
  // poprawienia wspolrzednych danego landmarku (URL niesie wspolrzedne
  // wprost w ?p=, wiec poprawienie bazy ZMIENIA sam URL - stary,
  // zaindeksowany link staje sie "nieaktualny" i wskazuje na zle
  // miejsce, dopoki Google go ponownie nie zaindeksuje). Sprawdzamy
  // wlasna, biezaca baze landmarkow po nazwie - jesli link niesie
  // wyraznie inna wspolrzedna niz to, co baza uwaza teraz za
  // poprawne, ufamy bazie, nie staremu URL-owi.
  function findNamedPoiRecordByName(name) {
    if (!name) return null;
    const records = window.OMAP_NAMED_POI_INDEX?.records;
    if (!Array.isArray(records)) return null;
    return records.find(r => r.name === name) || null;
  }

  // Mapowanie typu landmarku z wlasnej bazy na najblizszy, dobrze
  // wspierany typ Schema.org - dla nierozpoznanych typow bezpieczny,
  // ogolny "TouristAttraction" (pasuje do kazdego POI).
  const PLACE_SCHEMA_TYPE_MAP = {
    museum: "Museum",
  ...
  function setPlaceStructuredData(record) {
    clearPlaceStructuredData();
    if (!record?.name || !Number.isFinite(record.lat) || !Number.isFinite(record.lon)) return;

    const schemaType = PLACE_SCHEMA_TYPE_MAP[record.type] || "TouristAttraction";
    const data = {
    ...
    if (record.city) {
      ...
      if (record.voivodeship) {
        data.address.addressRegion = record.voivodeship;
      }
    }

    const script = document.createElement("script");
    script.type = "application/ld+json";
    script.id = "place-structured-data";
    script.textContent = JSON.stringify(data);
    document.head.appendChild(script);
  }

  function clearPlaceStructuredData() {
    document.getElementById("place-structured-data")?.remove();
  }

  function correctStaleLandmarkUrl(shared) {
    if (!shared?.label) return shared;
    const match = findNamedPoiRecordByName(shared.label);
    ...
    const dLat = toRad(match.lat - shared.lat);
    const dLon = toRad(match.lon - shared.lon);
    const a =
      Math.sin(dLat / 2) ** 2 +
      Math.cos(toRad(shared.lat)) *
        Math.cos(toRad(match.lat)) *
        Math.sin(dLon / 2) ** 2;
    const distanceKm = 2 * R * Math.asin(Math.sqrt(a));

    if (distanceKm < 0.05) return shared;

    console.info(
      `Wykryto nieaktualny link dla "${shared.label}" (${distanceKm.toFixed(2)}km od aktualnej pozycji w bazie) - poprawiam automatycznie.`
    );
    return { ...shared, lat: match.lat, lon: match.lon };
  }

  function loadSharedPlaceFromUrl() {
    let shared = window.OMAP_URL_STATE?.readPlaceFromUrl();
    if (!shared || !Number.isFinite(shared.lat) || !Number.isFinite(shared.lon)) return;
    shared = correctStaleLandmarkUrl(shared);

    // Tytul strony ustawiony NATYCHMIAST, synchronicznie, z wlasnej,
    // juz zaladowanej bazy landmarkow - bez czekania na jakiekolwiek
    // zapytanie sieciowe. Wczesniej appka czekala na odpowiedz
    // Nominatim (fetchPlaceByNameNear) zanim document.title dostawal
    // sensowna wartosc - dla crawlera Google, ktory ma ograniczony
    // czas/zasoby na wykonanie JS, mogło to oznaczac zaindeksowanie
    // strony zanim tytul zdazyl sie ustalic ("zeskanowano, ale nie
    // zindeksowano"). Sieciowe wzbogacenie (adres, Wikipedia) nadal
    // dzieje sie normalnie, w tle, ponizej.
    const namedPoiMatch = findNamedPoiRecordByName(shared.label);
    if (namedPoiMatch) {
      document.title = buildPageTitle({
        name: namedPoiMatch.name,
        type: namedPoiMatch.type
      });
      setPlaceStructuredData(namedPoiMatch);
    } else {
      clearPlaceStructuredData();
    }
  ...
  function initializeAutocomplete() {
      ...
      items.forEach(result => {
        ...
        if (result.__myLocationOption) {
          ...
          if (Number.isFinite(lat) && Number.isFinite(lon)) {
            const placeNameKey = getPlaceNameKey(result, { lat, lng: lon });
    ...
    const renderHistory = () => {
      ...
      history.forEach(entry => {
        ...
        if (Number.isFinite(lat) && Number.isFinite(lon)) {
          const placeNameKey = getPlaceNameKey(entry, { lat, lng: lon });
          ...
          openSearchPlaceThroughService(
            ...
            },
            {
              query:
                el.searchInput?.value ||
                entry.label,
              // Wpis historii wyszukiwarki ma juz zapisane dane
              // miejsca z momentu wyszukania - uzywamy ich
              // BEZPOSREDNIO (jak przy ocenach/ulubionych/historii
              // panelu miejsc), zamiast czystego odwrotnego
              // geokodowania, ktore moglo trafic w zupelnie inny
              // obiekt niz ten, ktory user faktycznie znalazl.
              reverse: false,
  ...
  function getSearchResultMatchScore(query, result) {
    ...
    else if (display.startsWith(normalizedQuery)) score = 0.95;
    else if (title.startsWith(normalizedQuery)) score = 0.9;
    else if (title.includes(normalizedQuery)) score = 0.82;
    else if (display.includes(normalizedQuery)) score = 0.74;
    else {
    ...
    // Granica administracyjna (np. "gmina Gdańsk", "powiat gdański")
    // o tej samej lub podobnej nazwie co prawdziwe miasto/miejscowosc
    // czesto ma NIEMAL identyczny wynik dopasowania tekstowego -
    // ktora z nich appka wtedy pokaze bylo w praktyce przypadkowe
    // (zalezalo od kolejnosci zwroconej przez Nominatim). Odejmujemy
    // wyrazny margines, zeby granica administracyjna wygrywala TYLKO
    // wtedy, gdy naprawdę nic innego (miasto/miejscowosc/POI) nie
    // pasuje choc trocz podobnie dobrze - ten sam sposob mysleania co
    // przy defibrylatorach ponizej.
    const isAdminBoundary = result.class === "boundary" && result.type === "administrative";
    if (isAdminBoundary) score -= 0.15;
...
function showUserLocationMarker(lngLat) {
  ...
  // Zapamietuje miejsce, do ktorego trzeba wrocic po akcji na koncie
  // (np. proba dodania komentarza bez zalogowania -> przekierowanie
  // do logowania -> po zalogowaniu chcemy wrocic do TEGO miejsca, nie
  // do glownego menu).
  let pendingPlaceReturn = null;

  function setPendingPlaceReturn(placeSnapshot, lngLat) {
    if (!placeSnapshot || !lngLat) return;
    pendingPlaceReturn = { placeSnapshot, lngLat };
  }

  function consumePendingPlaceReturn() {
    if (!pendingPlaceReturn) return false;
    const { placeSnapshot, lngLat } = pendingPlaceReturn;
    pendingPlaceReturn = null;
    openKnownPlaceOnMap(placeSnapshot, lngLat);
    return true;
  }
  ...
  async function fetchPlaceByNameNear(name, lat, lon, signal, languageOverride) {
    if (!name) return null;
    ...
    requestUrl.searchParams.set("extratags", "1");
    requestUrl.searchParams.set("namedetails", "1");
    requestUrl.searchParams.set("accept-language", languageOverride || state.language);
  ...
  function createPlaceCardLegacy(place, lngLat) {
    ...
    if (commentsUi) {
      card.appendChild(commentsUi.section);
      window.OMAP_COMMENTS?.loadForPlace(window.OMAP_FAVORITES?.getFavoriteKey(place, lngLat), commentsUi);
    }
  ...
  function getPlaceNameKey(place, lngLat, toleranceKm) {
    // Dla znanych landmarkow z wlasnej bazy (search-v2/named-poi) -
    // uzywamy klucza opartego na NAZWIE Z BAZY, nie na wspolrzednych z
    // biezacego obiektu. Wspolrzedne moga sie zmienic (np. po
    // poprawieniu bledu geokodowania), a klucz oparty na nich wtedy
    // "gubi" zapisana wczesniej wlasna nazwe uzytkownika. Prefiks
    // "poi-name:" zapobiega kolizji z istniejacymi kluczami opartymi
    // na wspolrzednych.
    //
    // Dopasowanie GLOWNIE po BLISKOSCI WSPOLRZEDNYCH, nie po nazwie -
    // rozne zrodla (etykieta z warstwy wektorowej mapy przy
    // kliknieciu, Nominatim, wlasna baza) nadaja temu samemu miejscu
    // czesto ZUPELNIE inaczej brzmiace nazwy (nie tylko drobne roznice
    // formatowania), wiec samo porownanie tekstu - nawet rozmyte - nie
    // wystarcza. Bliskosc geograficzna jest dużo bardziej niezawodna,
    // bo wszystkie zrodla i tak wskazuja na (w przyblizeniu) ten sam
    // punkt na mapie.
    const lat = Number(place?.lat ?? lngLat?.lat);
    const lon = Number(place?.lon ?? lngLat?.lng);
    const records = window.OMAP_NAMED_POI_INDEX?.records;

    if (Array.isArray(records) && Number.isFinite(lat) && Number.isFinite(lon)) {
      // Domyslnie ciasny promien (150m) - wystarcza do rozpoznania
      // "to ten sam punkt", ale NIE do "to gdzies w tym samym
      // miescie". Szerszy promien (1km) jest potrzebny TYLKO w jednym
      // konkretnym przypadku (klikniecie na duzy obiekt przez menu
      // kontekstowe "Informacje") - jesli uzyty wszedzie, kazdy wynik
      // wyszukiwania/historii w promieniu 1km od np. centrum Gdanska
      // dostawal BLEDNIE tozsamosc calego miasta.
      const PROXIMITY_MATCH_KM = toleranceKm || 0.15;
      const toRad = deg => (deg * Math.PI) / 180;
      ...
      for (const record of records) {
        ...
        const distance = haversineKm(lat, lon, record.lat, record.lon);
        if (distance < closestDistance) {
      ...
      // Wymagamy wyraznej przewagi najblizszego kandydata (jak przy
      // migracji osieroconych nazw) - w gesto zabudowanych obszarach
      // (kilka landmarkow blisko siebie) unikamy ryzyka przypisania
      // do niewlasciwego miejsca.
      if (
        closest &&
        closestDistance <= PROXIMITY_MATCH_KM &&
        secondClosestDistance >= closestDistance * 2
      ) {
        return `poi-name:${closest.name}`;
      }
    }

    // Zapasowo - rozmyte dopasowanie po nazwie, dla przypadkow gdy
    // wspolrzedne w biezacym obiekcie sa nieznane albo zbyt dalekie
    // (np. stary, jeszcze nie naprawiony blad geokodowania), ale
    // nazwa i tak wyraznie wskazuje na znany landmark.
    if (place?.name && Array.isArray(records)) {
      const normalizedPlaceName = normalizeSearchText(place.name);
      const match = records.find(
        r => stringSimilarity(normalizeSearchText(r.name), normalizedPlaceName) >= 0.75
      );
      if (match) return `poi-name:${match.name}`;
    }
  ...
  // Jednorazowa migracja: przed powyzsza poprawka klucz do wlasnych
  // nazw byl oparty WYLACZNIE na wspolrzednych. Kazda poprawka
  // wspolrzednej landmarku w bazie (a bylo ich w tej sesji sporo)
  // "gubila" zapisana wczesniej wlasna nazwe - appka szuka juz pod
  // NOWYM kluczem (nowa wspolrzedna), a nazwa lezy pod starym.
  // Szukamy dla kazdego takiego osieroconego wpisu najblizszego
  // landmarku w AKTUALNEJ bazie i, jesli jest wystarczajaco blisko,
  // przenosimy nazwe pod nowy, stabilny klucz oparty na nazwie.
  function migrateOrphanedCustomPlaceNames() {
    const records = window.OMAP_NAMED_POI_INDEX?.records;
    if (!Array.isArray(records) || !records.length) return;
    if (!state.customPlaceNames || typeof state.customPlaceNames !== "object") return;

    const MAX_MIGRATION_DISTANCE_KM = 20;
    const toRad = deg => (deg * Math.PI) / 180;
    const haversineKm = (lat1, lon1, lat2, lon2) => {
      const R = 6371;
      const dLat = toRad(lat2 - lat1);
    ...
    for (const key of Object.keys(state.customPlaceNames)) {
      // Interesuja nas tylko stare klucze-wspolrzedne "lat,lon" -
      // pomijamy klucze juz zmigrowane (poi-name:...) i inne formaty
      // (osm_type:osm_id, favorite key).
      const match = key.match(/^(-?\d+\.\d+),(-?\d+\.\d+)$/);
      if (!match) continue;

      const keyLat = Number(match[1]);
      const keyLon = Number(match[2]);
      ...
      for (const record of records) {
        if (!Number.isFinite(record.lat) || !Number.isFinite(record.lon)) continue;
        const distance = haversineKm(keyLat, keyLon, record.lat, record.lon);
        if (distance < closestDistance) {
          secondClosestDistance = closestDistance;
          closestDistance = distance;
          closest = record;
      ...
      if (!closest || closestDistance > MAX_MIGRATION_DISTANCE_KM) continue;

      // Bezpiecznik: w gesto zabudowanych obszarach (kilka
      // landmarkow blisko siebie) "najblizszy" moze byc ZLYM
      // dopasowaniem. Wymagamy, zeby najblizszy kandydat byl
      // WYRAZNIE blizej niz drugi najblizszy (co najmniej 2x) - w
      // przeciwnym razie wolimy zostawic wpis osierocony niz
      // ryzykowac przypisanie nazwy do niewlasciwego miejsca.
      if (secondClosestDistance < closestDistance * 2) continue;

      const newKey = `poi-name:${closest.name}`;
      // Nie nadpisujemy, jesli pod nowym kluczem juz cos jest
      // (np. uzytkownik juz recznie ustawil nazwe po naszej
      // wczesniejszej poprawce).
      if (state.customPlaceNames[newKey]) continue;

      state.customPlaceNames[newKey] = state.customPlaceNames[key];
    ...
    if (migratedCount > 0) {
      ...
      // WAZNE: nie tylko zapisujemy lokalnie, ale tez wysylamy do
      // chmury - w przeciwnym razie nastepne pobranie (kazda zmiana
      // teraz od razu sciaga tez dane z innych urzadzen) przywroci
      // STARA, niescalona wersje z chmury i "cofnie" te migracje,
      // dajac wrazenie ze naprawa "zadziala przez chwile i wraca".
      window.OMAP_ACCOUNT?.markLocalSyncChange();
      console.info(`Odzyskano ${migratedCount} własnych nazw miejsc po poprawkach współrzędnych w bazie.`);
    }
  }
...
// ===== KONIEC ZMODYFIKOWANEJ FUNKCJI =====
  ...
  window.OMAP_PLACE_SERVICE?.configure({
    async open(event) {
      ...
      // Ulubione i historia MAJA juz zapisane dane miejsca z momentu
      // dodania (nazwa, adres itd) - uzywamy ich BEZPOSREDNIO, tak
      // jak przy ocenach/komentarzach (placeSnapshot), zamiast
      // czystego odwrotnego geokodowania po samych wspolrzednych.
      // Wczesniej fallback na forceReverse (gdy brakowalo "silnego"
      // identyfikatora jak osm_type+osm_id) mogl trafic w zupelnie
      // inny obiekt niz ten, ktory user faktycznie zapisal.
      if (
        hasExactIdentity ||
        event.source === "favorite" ||
        event.source === "favorites" ||
        ...
        showSelectedPlaceInformation(snapshotPlace);

        // Zapisana migawka moze nie miec wypelnionych pol potrzebnych
        // do wyzwolenia sekcji Wikipedii (type/extratags.wikipedia) -
        // jesli mamy osm_type+osm_id, dociagamy w tle PELNE, swieze
        // dane po DOKLADNYM id (nie po wspolrzednych - zero ryzyka
        // trafienia w inny obiekt) i ponownie renderujemy, TYLKO
        // jesli user nadal ogląda to samo miejsce.
        if (place.osm_type && place.osm_id) {
          fetchPlaceByOsmId(place.osm_type, place.osm_id)
            .then(fullPlace => {
              if (
                fullPlace &&
                state.placePanelLngLat?.lat === lat &&
                state.placePanelLngLat?.lng === lon &&
                !el.placePanel?.hidden
              ) {
                showSelectedPlaceInformation({
                  ...fullPlace,
  ...
  async function openMapInformationThroughService(
    ...
    const placeNameKey = getPlaceNameKey({ lat, lon }, lngLat, 1);
    const customName = state.customPlaceNames[placeNameKey];
  ...
  async function loadMyRatingsActivity() {
    ...
    try {
      ...
      const [ratings, comments] = await Promise.all([
        transport.fetchMyRatings(myPubKeyHex),
        transport.fetchMyComments(myPubKeyHex).catch(() => [])
      ]);

      const activityEntries = [
        ...ratings.map(r => ({ ...r, activityType: "rating", activityAt: r.ratedAt })),
        ...comments.map(c => ({ ...c, activityType: "comment", activityAt: c.commentedAt }))
      ];
      activityEntries.sort((a, b) => new Date(b.activityAt) - new Date(a.activityAt));
      ...
      activityEntries.forEach(entry => {
        ...
        if (entry.activityType === "comment") {
          stars.textContent = `💬 ${entry.text.slice(0, 60)}${entry.text.length > 60 ? "…" : ""}`;
        } else {
        ...
        removeButton.addEventListener("click", async () => {
          ...
          try {
            ...
            if (entry.activityType === "comment") {
              await transport.deleteComment(nostrPrivKeyBytes, entry.id);
            } else {

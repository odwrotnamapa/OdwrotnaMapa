(function () {
  ...
  function returnFromAccountToMenu() {
    ...
    closeAccount();
    if (ctx.consumePendingPlaceReturn?.()) return;
    ctx.openMenuHome();
  }
  ...
  async function computeAndShowIdentity() {
    ...
    try {
      ...
      if (ctx.el.accountPublicId) {
        ...
        // zbliżonej do identyfikatora filmu na YouTube. Przycisk
        // "Kopiuj" kopiuje TĘ SAMĄ, skróconą wersję - nie pełny npub.
  ...
  // Zapewnia, ze publiczne metadane (kind 0, widoczne np. przy
  // komentarzach) istnieja - jesli uzytkownik ustawil nazwe ZANIM ta
  // funkcja w ogole powstala, nigdy nie zostala opublikowana
  // publicznie (tylko w prywatnym, szyfrowanym blobie). Wolane przy
  // logowaniu I przy zwyklym starcie appki dla juz zalogowanych
  // (patrz app.js) - bez tego istniejacy uzytkownicy musieliby sami
  // wejsc i ponownie zapisac profil, zeby ich nazwa zaczela byc
  // widoczna dla innych.
  async function ensurePublicProfileMetadata(accountCtx) {
    if (!accountCtx) return;
    const currentProfile = getStoredProfile();
    if (!currentProfile.name) return;
    try {
      await accountCtx.transport.publishProfileMetadata(
        accountCtx.nostrPrivKeyBytes,
        currentProfile
      );
    } catch (error) {
      console.error("Nie udało się opublikować publicznych metadanych profilu:", error);
    }
  }

  async function ensurePublicProfileMetadataFromSeedWords(seedWords) {
    if (!seedWords) return;
    try {
      const accountCtx = await deriveAccountContext(seedWords);
      await ensurePublicProfileMetadata(accountCtx);
    } catch (error) {
      console.error("Nie udało się zapewnić publicznych metadanych profilu:", error);
    }
  }
  ...
  function getCheckedSyncScopes() {
    // Synchronizujemy zawsze wszystko - bez osobnych checkboksow do
    // wybierania zakresu. Konto to jedna, spojna rzecz do
    // zsynchronizowania, nie zestaw opcjonalnych fragmentow.
    return ["favorites", "colors", "placeNames", "history"];
  }
  ...
  async function performPush(scopes, options) {
    ...
    try {
      ...
      ctx.safeSet(ctx.CONFIG.storageKeys.syncLocalSyncedAt, new Date().toISOString());
  ...
  // z chmury.
  // Kazda lokalna zmiana (dodanie ulubionego, zmiana koloru itd) sama
  // sie wysyla - dokladnie tak jak dziala to przy ocenach (kazda
  // zmiana zapisuje sie od razu, bez czekania na jakikolwiek przycisk
  // czy pozniejsze porownywanie znacznikow czasu). Krotki debounce
  // (1.5s) laczy szybko po sobie nastepujace zmiany w jedno wyslanie,
  // zamiast robic osobne zapytanie przy kazdym pojedynczym
  // kliknieciu.
  let immediatePushTimer = null;

  function markLocalSyncChange() {
  ...
  // Synchronizacja dziala teraz WYLACZNIE na wyrazne kliknniecie
  // przycisku odswiezania (performSafeSync) - bez zadnego cyklicznego
  // timera dzialajacego niewidocznie w tle. Ten timer byl zrodlem
  // nieprzewidywalnego nadpisywania swiezych lokalnych zmian (np.
  // dodania ulubionego) - synchronizacja w tle mogla trafic w
  // niekorzystnym momencie, zanim lokalna zmiana zdazyla sie
  // "utrwalic". Funkcja zostaje jako pusta (no-op) zamiast usuwac ja
  // calkowicie, zeby nie psuc pozostalych miejsc, ktore ja wywoluja.
  function scheduleAutoSyncCheck() {}

  // Kazda lokalna zmiana wysyla sie sama juz w markLocalSyncChange -
  // ten przycisk (jedyny widoczny w panelu Konto) sluzy WYLACZNIE do
  // recznego pobrania danych z innych urzadzen. Zaden wybor
  // kierunku, zaden warunek - proste, bezposrednie pobranie, tak jak
  // prosil uzytkownik.
  async function performSafeSync(options) {
    ...
    try {
      ...
      console.error("Pobranie profilu nie powiodło się:", error);
    }
    ...
    try {
      ...
      console.error("Pobranie nie powiodło się:", error);
      if (!silent) showAccountMessage(t.accountReceiveFailed, "error");
      return null;
    }
  }
  ...
  async function saveProfile(name, avatar) {
    ...
    try {
      ...
      // Publiczna wersja (kind 0) - zeby inni uzytkownicy widzieli tu
      // ustawiona nazwe np. przy Twoich komentarzach. Celowo nie
      // przerywamy calej operacji, jesli TYLKO to sie nie uda -
      // wlasny, szyfrowany zapis juz sie udal i to jest najwazniejsze.
      try {
        await accountCtx.transport.publishProfileMetadata(accountCtx.nostrPrivKeyBytes, { name, avatar });
      } catch (publicError) {
        console.error("Nie udało się opublikować publicznych metadanych profilu:", publicError);
      }
  ...
  function initializeAccountEventListeners() {
    ...
    ctx.el.accountSyncRefreshButton?.addEventListener("click", () => performSafeSync({ silent: false }));
    ...
    ctx.el.accountPublicId?.addEventListener("click", async () => {
      const shortId = ctx.el.accountPublicId?.textContent;
      if (!shortId) return;
      const t = ctx.text[ctx.state.language];
      try {
  ...
  window.OMAP_ACCOUNT = {
    ...
    getStoredProfile,
    storeProfileLocally,
    ensurePublicProfileMetadata,
    ensurePublicProfileMetadataFromSeedWords,
    renderProfileUI,
    refreshAccountUI,
